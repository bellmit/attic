 ...
 * fallback.
 */
public class ErrorPage extends SimpleDsl<ErrorPageDriver, TestContext> {
    public ErrorPage(Supplier<? extends ErrorPageDriver> driverFactory, TestContext testContext) {
        super(driverFactory, testContext);
    }

    /**
     * Ensure that a Jetty error page is shown. This DSL method has two required parameters: <ul> <li><var>error</var>:
     * the error type message shown at the top of the page</li> <li><var>message</var>: the error message body shown in
     * the center of the page</li> </ul>
     */
