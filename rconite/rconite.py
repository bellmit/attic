...
def decode(message):
    ...
    try:
        ...
        s1, sep, remainder = message[8:].partition('\x00')
        if sep != '\x00':
            raise ValueError('incomplete message: %r' % (message,))
        s2, sep, remainder = remainder.partition('\x00')
        if sep != '\x00':
            raise ValueError('incomplete message: %r' % (message,))
        if remainder != '':
...
class RconChannel(object):
    ...
    def _read_fully(self, length):
        ...
        while len(read) < length:
            ...
            if data == '': # oops, premature end of stream
...
class RconClient(object):
    ...
    def _converse(self, type, s1):
        ...
        message = encode(request_id, type, s1)
