...
Instantiate a tool, specifying the database `uri <http://www.sqlalchemy.org/docs/05/dbengine.html#create-engine-url-arguments>`_ and directory that contains migration
files:
...
class MigrationTool(object):
    ...
    def __init__(self, dburi, migration_dir, schema_table='sql_migration'):
        ...
        self.table = Table(schema_table, self.metadata,
    ...
    def run_migrations(self):
        ...
        for mfile in migrations:
            try:
                ...
                log.debug("Successfully ran migration %s", mfile)
            except Exception, e:
